...
sequenceDiagram
    ...
    F->>G: Call Gemini (gemini-2.0-flash-lite or similar free-tier model)
    G-->>F: Answer
    F-->>W: { reply }
...
3. Call Gemini's `generateContent` REST endpoint (model: `gemini-2.0-flash-lite` or similar free-tier model — cheap/free, fast, plenty capable for grounded Q&A over a few KB of data).
